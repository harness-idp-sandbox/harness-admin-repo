...
# run app
java -jar target/parson-test-three-1.0-SNAPSHOT.jar
...
docker buildx build   --platform linux/amd64   -t pkg.harness.io/parson-test-three:latest   --push .
```
...
# watch rollout
kubectl rollout status deploy/parson-test-three -n default

# get external endpoint
kubectl get svc parson-test-three-svc -n default
```
...
```bash
kubectl port-forward svc/parson-test-three-svc 8080:80 -n default
...
kubectl rollout restart deploy/parson-test-three -n default
kubectl rollout status deploy/parson-test-three -n default
```
...
├─ src/
│  ├─ main/
│  │  ├─ java/com/harness/parsontestthree/
│  │  │  ├─ App.java
...
│  │        └─ index.html
│  └─ test/
│     └─ java/com/harness/parsontestthree/AppTest.java
├─ kubernetes/
│  ├─ deployment.yml
│  └─ service.yml
├─ Dockerfile
